...
def bus(busGraph, start, end):
	...
	bus = pois_from_polygon(box(minstartLon, minstartLat, maxstartLon, maxstartLat), tags=tags)
	# print(bus.columns)
	busStopStart = None
	...
	bus = pois_from_polygon(box(minendLon, minendLat, maxendLon, maxendLat), tags=tags)
	# print(bus.columns)
	busStopEnd = None
	busStopEndName = None
	for name in bus["name"]:
...
# fig, ax = ox.plot_graph_route(graph, nodepath, fig_height=10, fig_width=10, orig_dest_node_color='green', route_color='green', show=False, close=False)
...
m = ox.plot_route_folium(graph, nodepath, route_color='green', route_opacity=0) # opacity 0 just to make the driving line disappear
...
folium.PolyLine([line[0], start], color="blue", weight=2.5, opacity=1, dasharray="4").add_to(m)
folium.PolyLine([line[-1], end], color="blue", weight=2.5, opacity=1, dasharray="4").add_to(m)
m